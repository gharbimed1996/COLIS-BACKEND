...
public class EntrepriseController {
    ...
    @PostMapping("/create")
    public String createEntreprise(@RequestBody EntrepriseDTO entrepriseDTO) {

        return userService.addentrOrg(entrepriseDTO);
    }
    @PutMapping("/update/{userId}")
    public String updateEntreprise(@PathVariable String userId, @RequestBody EntrepriseDTO updatedEntrepriseDTO) {
        try {
            userService.updateEntreprise(userId, updatedEntrepriseDTO);
            return "Enterprise user updated successfully";
        } catch (Exception e) {
            // Log the exception for debugging
            LOG.error("Error updating enterprise user", e);
            return "Error updating enterprise user: " + e.getMessage();
        }
    }
    ...
    public ResponseEntity<String> deleteEntreprise(@PathVariable String userId) {
        userService.deleteEntreprise(userId);
        return ResponseEntity.ok("Enterprise user deleted successfully");
    }
