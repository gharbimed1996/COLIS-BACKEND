...
public class LivreurController {
    ...
    @Autowired
    private LivreurService userService;
    @Autowired
    private KeycklockConfig config;

    private static final Logger LOG = org.slf4j.LoggerFactory.getLogger(LivreurController.class);



    @PostMapping("/create")
    public String createuser(@RequestBody LivreurDTO userDTO) {

        return userService.addLivreur(userDTO);
    }
    @PostMapping("/login")
    ...
    public ResponseEntity<String> deleteLivreur(@PathVariable String userId) {
        userService.deleteLivreur(userId);
